...
`verify:language` checks configured-name address, removal of formal `sir` defaults and canned factual asides, common Jarvis replies, natural entity aliases, directional device names, aggregate camera counts and other whole-home questions, unavailable states, read-only control handling, and persona guardrails without requiring Home Assistant or Ollama.
